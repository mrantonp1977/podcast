...
export const getPodcastBySearch = query({
  ...
  handler: async (ctx, args) => {
    ...
    // If search is not provided or is an empty string, return all podcasts in descending order
    ...
    const titleSearch = await ctx.db
      .query("podcasts")
      .withSearchIndex("search_title", (q) => q.search("podcastTitle", search))
      .take(10);
    ...
    const descriptionSearch = await ctx.db
      .query("podcasts")
      .withSearchIndex("search_body", (q) => q.search("podcastDescription", search))
      .take(10);
